...
# Рандом числа для пинга
def random_choice():
    choices = ['16,68',
              '5,97']

# Рандом цитаты для ставки		
def random_quote():
    quotes = ['Не переставай верить в свои силы, ведь фортуна уже готова улыбнуться тебе.',
              'В казино удача находит того, кто ее ждет.',
...
def generate_keyboard(page: int, refs: list, total_pages: int, per_page: int):
    ...
    keyb.add(types.InlineKeyboardButton(text="🔍 Поиск", callback_data='search_refferals'), 
           types.InlineKeyboardButton(text="<- Назад", callback_data='ref_panel'))
...
async def transfer(amount, us_id):
    ...
    bal = float(bal)
    amount = float(amount)
    keyb = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("🏷️Закрыть", callback_data='close'))
    keyb = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⚡ Перейти к пользователю", url=f"tg://user?id={us_id}"))
    if bal < amount:
        try:
            await bot.send_message(us_id, f"<blockquote><b>🔔 Уведомление.</b></blockquote>\n\n<b><blockquote>Ваша выплата ⌊ {amount}$ ⌉ будет зачислена вручную администратором!</blockquote></b>", reply_markup=keyb)
        except:
            pass
        await bot.send_message(config.LOGS_ID, f"<blockquote><b>🔔 Уведомление.</b></blockquote>\n\n<b>Мало суммы в казне для выплаты!</b>\n\n<b>Пользователь: {us_id}\nСумма: {amount}$</b>", reply_markup=keyb)
        return
    try:
        ...
        await bot.send_message(config.LOGS_ID, f"<blockquote><b>🧾 Перевод средств!</b></blockquote>\n<b><b>💠 Сумма: {amount} USDT</b>\n<b>⭐ Пользователю: {us_id}</b>", reply_markup=keyb)
...
async def create_check(amount, userid):
    ...
    if bal < amount:
        try:
            await bot.send_message(userid, config.LOGS_ID, f"<blockquote><b>🔔 Уведомление.</b></blockquote>\n\n<b><blockquote>Ваша выплата ⌊ {amount}$ ⌉ будет зачислена вручную администратором!</blockquote></b>", reply_markup=keyb)
        except:
            pass
        await bot.send_message(config.LOGS_ID, f"<blockquote><b>🔔 Уведомление.</b></blockquote>\n\n<b>Мало суммы в казне для выплаты!</b>\n\n<b>Пользователь: {us_id}\nСумма: {amount}$</b>", reply_markup=keyb)
        return
    ...
    await bot.send_message(config.LOGS_ID, f"<blockquote><b>🧾 Создан чек!</b></blockquote>\n<b>💠 Сумма: {amount} USDT</b>\n<b>⚡ Прикрепен за юзером: {userid}</b>", reply_markup=keyb)
...
async def poshel_nahuy_telebot(message: types.Message, state: FSMContext):
    ...
    if check:
       msg = await message.answer(f"<b>👋 Добро пожаловать в реферального бота {config.CASINO_NAME}!</b>\n\n<b>📊 Ваша статистика\n</b><blockquote><b>└ 📋 Общая сумма ставок - {total_bets_summ}$\n└ 🏆 Сумма выигрышей - {total_wins_summ}$\n└ 🚫 Сумма проигрышей - {total_lose_summ}$</b></blockquote>", reply_markup=kb.menu(message.from_user.id))
    else:
        keyb = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("⚡ Подписаться на Игровой канал", url=config.BETS_LINK))
        msg = await message.answer("<blockquote><b>❌ Чтобы продолжить вы должны быть подписаными на канал, после того как вы подписались пропишите заново /start</b></blockquote>", reply_markup=keyb)
...
async def ref_search(message: types.Message, state: FSMContext):
    ...
    if not user:
        ...
        if user[4] != message.from_user.id:
            ...
            msg = await message.answer(f"<blockquote><b>✔️ {message.text} ваш реферал!</b></blockquote>", reply_markup=kb.back("refs"))
...
async def deposit_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        keyb = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("⚡ Оплатить", url=invoice), InlineKeyboardButton("<- Назад", callback_data='popol'))
...
async def calls(call: types.CallbackQuery, state: FSMContext):
    ...
    if check:
        ...
        await call.message.edit_text("<blockquote><b>❌ Чтобы продолжить вы должны быть подписаными на канал снизу, после того как вы подписались пропишите заново команду /start</b></blockquote>", reply_markup=keyb)
    ...
    if call.data == 'profile':
        ...
        await call.answer()
        await call.message.edit_text(f"""<b>⚡ Профиль {call.from_user.first_name} // {call.from_user.id}\n</b> <blockquote><b>Информация:\n├ WinRate - <code>{winrate}%</code>\n├ Ставки за все время - <code>{total_bets_summ}$</code> за <code>{total_bets}</code> игр\n└ Дата регистрации - <code>{formatted_date_str}</code>  (<code>{days_joined_text}</code>)</b></blockquote>""", reply_markup=kb.profile())
    elif call.data == 'menu':
        with sqlite3.connect("db.db") as conn:
            cursor = conn.cursor()
        ...
        await call.answer()
        await call.message.edit_text(f"<b>👋 Добро пожаловать в реферального бота {config.CASINO_NAME}!</b>\n\n<b>📊 Ваша статистика\n</b><blockquote><b>└ 📋 Общая сумма ставок - {total_bets_summ}$\n└ 🏆 Сумма выигрышей - {total_wins_summ}$\n└ 🚫 Сумма проигрышей - {total_lose_summ}$</b></blockquote>", reply_markup=kb.menu(call.from_user.id))
        ...
        await call.answer()
        await call.message.edit_text(f"<b>⚡ Статистика о нашем проекте\n</b>\n<blockquote><b>├ количество игр - <code>{total_games}</code> шт.\n├ Сумма общих выплат:\n├ <code>{formatted_wins}$</code>\n└ <code>{formatted_rub}₽</code></b></blockquote>", reply_markup=kb.back("menu"))
        ...
        await call.message.edit_text(f"<b>⚡ Акция\n</b>\n<blockquote><b>— приглашайте рефералов по вашей ссылке! Вывод доступен от 100 рефералов / 100 рефералов - 10$\nКоличество рефералов: <code>{total_refs}</code>\nРеф.Ссылка - <code>t.me/{bot_username.username}?start=ref_{call.from_user.id}</code></b></blockquote>", reply_markup=kb.ref())
    elif call.data == 'refs':
        with sqlite3.connect("db.db") as conn:
            cursor = conn.cursor()
        ...
        await call.answer()
        bot_username = await bot.get_me()
        await call.message.edit_text(f"<b>⚡ Реферальная панель\n</b>\n<blockquote><b>Вы будете получать <code>10%</code> от проигрыша вашего реферала вывод доступен от <code>0.2$</code>\nКоличество рефералов - <code>{total_refs}</code>\nPеферальный баланс - <code>{ref_balance}$</code>\nРеф. Ссылка - <code>t.me/{bot_username.username}?start=ref_{call.from_user.id}</code></b></blockquote>", reply_markup=kb.ref())
    elif call.data == 'refs':
        with sqlite3.connect("db.db") as conn:
            cursor = conn.cursor()
            refs = cursor.execute("SELECT * FROM users WHERE ref=?", (call.from_user.id,)).fetchall()
        ...
        await call.answer()
        await call.message.edit_text(f"<b>⚡ Панель кэшбек системы\n</b>\n<blockquote><b>В случае проигрыша вы получаете <code>7.5%</code> от суммы ставки вывод доступен от <code>0.2$</code>\nКэшбек-счет - <code>{cashback:.7f}$</code></b></blockquote>", reply_markup=kb.cashback())
        ...
        if call.from_user.id in config.ADMINS:
            with sqlite3.connect("db.db") as conn:
                ...
                await call.answer()
                await call.message.edit_text(f"<b>⚡ Админ-Панель\n </b>\n<blockquote><b>├ Пользователей - <code>{total_users}</code> шт.\n</b><b>├ Количество ставок - </b>~<b> <code>{total_bets}</code> шт. </b>[~ <code>{total_bets_summ}</code> <b>$</b>]\n<b>├ Выигрышей - </b>~<b> <code>{total_wins}</code> шт. </b>[~ <code>{total_wins_summ}</code> <b>$</b>]\n<b>└ Проигрышей - </b>~<b> <code>{total_loses}</code> шт. </b>[~ <code>{total_loses_summ}</code> <b>$</b>]</blockquote>", reply_markup=kb.admin())
        ...
        if call.from_user.id in config.ADMINS:
            ...
            if int(set_to) == 1:
                await bot.send_message(config.CHANNEL_ID, "<b>⚡ Приём ставок временно отключены администрацией!</b>")
            elif int(set_to) == 0:
                await bot.send_message(config.CHANNEL_ID, "<b>⚡ Приём ставок успешно включены администрацией!</b>")
    ...
    # InlineKeyboardButton("Проверить [✅]", callback_data='$'),
    InlineKeyboardButton("Вернуться в бота", url="https://t.me/Casinotesterka_bot"),
            )
            await bot.send_message(config.LOGS_ID, """<b>[🎰]</b> <b><blockquote>Система @ [🎲]\n 🟢/🔴</blockquote></b>
""", reply_markup=keyb)                    
        ...
        if call.from_user.id in config.ADMINS:
            ...
            keyb = InlineKeyboardMarkup(row_width=1).add(
    # InlineKeyboardButton("Проверить [✅]", callback_data='$'),
    InlineKeyboardButton("Вернуться в бота", url="http://t.me/Casinotesterka_bot"),
            )
            await bot.send_message(config.LOGS_ID, """<b>[🎰]</b> <b><blockquote>Система подкрут [🎲]\n 🟢/🔴</blockquote></b>
""", reply_markup=keyb)   
        ...
        if call.from_user.id in config.ADMINS:
            ...
            keyb = InlineKeyboardMarkup(row_width=1).add(
     InlineKeyboardButton(" ✔️ Проверить приписку", callback_data='podkrut1'),
    InlineKeyboardButton(" ❓ Как сделать ставку", url=config.KAK_SDELAT),
    InlineKeyboardButton(" 🎰 Команды для игр", url=config.KOMANDI_IGRI),
    InlineKeyboardButton(" ⚡ Сделать ставку", url=config.BET_URL),
            )
            await bot.send_message(config.CHANNEL_ID, """<b>[🎰] Играй и зарабатывай с @Casinotesterka_bot.

Добавь приписку @Casinotesterka_bot и получай дополнительный бонус за активность! С каждой ставкой с припиской тебе будет добавляться специальный коэффициент, который увеличивает твой приз! 🚀

Обязательно условие в описание профиля поставить следующий текст:</b>

<blockquote><b>В @Casinotesterka_bot каждый ход — шанс удвоить свой выигрыш! [🎰]. Коэф — x2</b></blockquote>""", reply_markup=keyb)    
    ...
    InlineKeyboardButton(" ⚡ Сделать ставку", url=config.BET_URL),
            ...
            await bot.send_message(config.CHANNEL_ID, """<b>Дорогие друзья хотели бы сообщить что мы пополнили казну очередной раз и теперь все ваши выигрыши будут начислены моментально на @CryptoBot.

Минимальная сумма ставки на данный момент 0.02$/2₽
                   
Также у нас действует акция 100 рефералов — 10$, свою реферальную ссылку вы сможете получить в боте @Casinotesterka_bot -> кнопка -> профиль -> Акция.
                                   
Актуальная ссылку на нашу техническую поддержку — @Saporv по всем вопросам обращайтесь сюда.
                                 
Желаем вам хорошего дня и удачных ставок.

С уважением @Casinotesterka_bot. ✨</b>""", reply_markup=keyb)
...
<b>👇 Прочитать его можно по кнопке снизу:</b>""", reply_markup=keyb)
        ...
        try:
            if page == 1:
                keyb.add(InlineKeyboardButton("-> Дальше", callback_data='tutorial:2'))
                await bot.send_message(call.from_user.id, """<b>👋 Привет, давай расскажу как сделать ставку!
    
<blockquote>Для начала тебе нужно совершить депозит в бота @CryptoBot если ты еще этого не сделал.</blockquote></b>""", reply_markup=keyb)
            elif page == 11:
                keyb.add(InlineKeyboardButton("-> Дальше", callback_data='tutorial:2'))
                await bot.edit_message_text("""<b>👋 Привет, давай расскажу как сделать ставку!

<blockquote>Для начала тебе нужно совершить депозит в бота @CryptoBot если ты еще этого не сделал.</blockquote></b>""", call.from_user.id, call.message.message_id, reply_markup=keyb)
            elif page == 2:
                keyb.add(InlineKeyboardButton("<- Назад", callback_data='tutorial:11'), InlineKeyboardButton("-> Дальше", callback_data='tutorial:3'))
                await call.message.edit_text(f"""<b>📝 Теперь вы должны выбрать на что хотите поставить!</b>\n
...
· <b>✊ Камень / ножницы / бумага -</b> <code>✌️ Ножницы - Побеждают бумагу.

✋ Бумага - Побеждает камень.

✊ Камень - Побеждает ножницы.</code>""", reply_markup=keyb)
            elif page == 3:
                keyb.add(InlineKeyboardButton("<- Назад", callback_data='tutorial:2'), InlineKeyboardButton("-> Дальше", callback_data='tutorial:4'))
                await call.message.edit_text(f"""<b>📍 После выбора необходимо оплатить счёт тем самым создав ставку!</b>
    
<blockquote><b>💎 Переходишь на оплату счету «{config.BET_URL}» -> Вводишь сумму ставки в нужной валюте -> Добавляешь комментарий, а именно ставка которую ты выбрал (Например больше) -> Нажимаешь оплатить счет и наблюдаешь над своей ставкой в канале ставок.</b></blockquote>
    
<b>✅ Вот и всё! Если у вас возникли вопросы или нашли баги обратитесь к {config.OWNER_LINK}.</b>""", reply_markup=keyb, disable_web_page_preview=True)
            elif page == 4:
                keyb.add(InlineKeyboardButton("<- Назад", callback_data='tutorial:3'))
                await call.message.edit_text(f"""<b>❓ Куда же приходит выплата в случае выигрыша?</b>
    
<blockquote><b>💹 Если вы выиграли то на ваш счёт @CryptoBot моментально должны прийти средства.</b></blockquote>


<b>🛂 В случае проблем с зачислением средств на ваш счёт обратитесь к администратору (<a href="{config.OWNER_LINK}">администрация</a>)</b>""", reply_markup=keyb, disable_web_page_preview=True)
        except:
            await call.answer("‼️Вы должны быть в нашем реферальном боте!", show_alert=True)
    elif call.data == 'control_user':
        if call.from_user.id in config.ADMINS:
            await call.answer()
            await call.message.edit_text("<blockquote><b>👤 Отправьте username пользователя!</b></blockquote>", reply_markup=kb.back("admin"))
        ...
        if call.from_user.id in config.ADMINS:
            ...
            with sqlite3.connect("db.db") as conn:
                ...
                conn.commit()
            await call.answer("✔️ успешно анулирован!", show_alert=True)
        ...
        keyb.add(InlineKeyboardButton("<- Назад", callback_data='admin'))
        ...
        if r['ok'] == True:
            await call.answer("🎉 Чек успешно удален с удержания!", show_alert=True)
            headers = {"Crypto-Pay-API-Token": config.CRYPTOPAY_TOKEN}
            r = requests.get("https://pay.crypt.bot/api/getChecks", headers=headers).json()
            keyb = InlineKeyboardMarkup(row_width=2)
            btns = []
        ...
        await call.message.edit_text("<blockquote><b>⚡ Введите сумму которую вы хотите вывести, от 0.1$</b></blockquote>", reply_markup=kb.back("admin"))
        
        await states.Withdraw.start.set()
        await state.update_data(msg_id=call.message.message_id)
    elif call.data == 'links':
        await call.answer("⚡ Временно не доступно.", show_alert=True)
    elif call.data == 'ping':
       await call.answer(f"🏓 {random_choice()} ping", show_alert=False)
    elif call.data == 'download_db':
       await call.answer(f"🗃️ database is generated. \n-> t.me/+jnP5A6v3Gg01ZDA6", show_alert=True)
    elif call.data == 'moder':
       await call.answer(f"⚡В разработке...", show_alert=False)
...
async def withdraw_handler(message: types.Message, state: FSMContext):
    ...
    try:
        summa = float(message.text)
        if summa < float(0.1):
            await message.answer("<blockquote><b>❌ Сумма меньше 0.1$.</b></blockquote>", reply_markup=kb.back("admin"))
            await message.delete()
            return
        else:
            ...
            if float(cb_balance) < summa:
                ...
                if summa >= float(1.12):
                    ...
                    keyb = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("⚡ Забрать средства", url=check), InlineKeyboardButton("<- Назад", callback_data='admin'))
                    await message.answer("<blockquote><b>✔️ Успешно! Заберите чек по кнопке ниже.</b></blockquote>", reply_markup=keyb)
                    await message.delete()
                    return
                else:
        ...
        await message.answer("<blockquote><b>❌ Вводить сумму нужно числами!</b></blockquote>", reply_markup=kb.back("admin"))
        await message.delete()
        return
...
# Способ получения через entities
    if message.entities:
...
def create_keyboard(check=None, summa=None):
    ...
    if check == None and summa == None:
        ...
        kak_button = InlineKeyboardButton(" ❓Как сделать ставку", url=config.KAK_SDELAT)
        KOMANDI_IGRI = InlineKeyboardButton(" 🎰 Команды для игр", url=config.KOMANDI_IGRI)
...
async def send_result_message(result, parsed_data, dice_result, coefficient, us_id, msg_id):
    ...
    if result:
        ...
        if cb_balance < winning_amount_usd:
            keyb = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⚡ Перейти к пользователю", url=f"tg://user?id={us_id}"))
            await bot.send_message(config.LOGS_ID, f"<blockquote><b>🔔 Уведомление.</b></blockquote>\n\n<b>Мало суммы в казне для выплаты!</b>\n\n<b>Пользователь: {us_id}\nСумма: {winning_amount_usd}$</b>", reply_markup=keyb)
            keyboard = create_keyboard()
            result_message = (
                f"<b><i>Поздравляю, вы выиграли!</i></b>\n\n"
                f"<blockquote><b><i>Победитель получит {winning_amount_usd:.2f}$ вручную администрацией</i></b></blockquote>\n\n<blockquote><b><i>Ставь ставку и испытывай удачу!</i></b></blockquote>\n\n<blockquote><b><i>-> 100 рефералов - 10$ в нашем боте @{bot_username}</i></b></blockquote>\n\n"
                f"<b><a href='{config.RULES_LINK}'>Правила и пользовательское соглашение</a> | <a href='{config.NEWS_LINK}'>Новостной канал</a> | <a href='https://t.me/{bot_username}'>Реферальный бот от нашего казино</a> | <a href='{config.OWNER_LINK}'>Техническая поддержка</a> | <a href='{config.PEREHOD_LINK}'>Ссылка для друзей</a></b>"
            )
        else:
            if winning_amount_usd >= 1000:
                ...
                result_message = (
                    f"<b><i>Поздравляю, вы выиграли!</i></b>\n\n"
                    f"<blockquote><b><i>Победитель получил {winning_amount_usd:.2f}$ на @CryptoBot</i></b></blockquote>\n\n<blockquote><b><i>Ставь ставку и испытывай удачу!</i></b></blockquote>\n\n<blockquote><b><i>-> 100 рефералов - 10$ в нашем боте @{bot_username}</i></b></blockquote>\n\n"
                    f"<b><a href='{config.RULES_LINK}'>Правила и пользовательское соглашение</a> | <a href='{config.NEWS_LINK}'>Новостной канал</a> | <a href='https://t.me/{bot_username}'>Реферальный бот от нашего казино</a> | <a href='{config.OWNER_LINK}'>Техническая поддержка</a> | <a href='{config.PEREHOD_LINK}'>Ссылка для друзей</a></b>"
                )
            else:
                ...
                result_message = (
                    "<b><i>Поздравляю, вы выиграли!\n\n</i></b>"                             f"<blockquote><b><i>Заберите свои {winning_amount_usd:.2f}$ по кнопке ниже!</i></b></blockquote>\n\n<blockquote><b><i>Ставь ставку и испытывай удачу!</i></b></blockquote>\n\n<blockquote><b><i>-> 100 рефералов - 10$ в нашем боте @{bot_username}</i></b></blockquote>\n\n"""
                    f"<b><a href='{config.RULES_LINK}'>Правила и пользовательское соглашение</a> | <a href='{config.NEWS_LINK}'>Новостной канал</a> | <a href='https://t.me/{bot_username}'>Реферальный бот от нашего казино</a> | <a href='{config.OWNER_LINK}'>Техническая поддержка</a> | <a href='{config.PEREHOD_LINK}'>Ссылка для друзей</a></b>"
                )
    else:
        usd_amount = parsed_data['usd_amount']
        ...
        result_message = (
            ...
            f"<blockquote><b><i>Вам начислен кэшбек {add_cashback:.7f}$ на баланс бота @{bot_username}</i></b></blockquote>\n\n"
            f"<blockquote><b><i>-> 100 рефералов - 10$ в нашем боте @{bot_username}</i></b></blockquote>\n\n"
            f"<b><a href='{config.RULES_LINK}'>Правила и пользовательское соглашение</a> | <a href='{config.NEWS_LINK}'>Новостной канал</a> | <a href='https://t.me/{bot_username}'>Реферальный бот от нашего казино</a> | <a href='{config.OWNER_LINK}'>Техническая поддержка</a> | <a href='{config.PEREHOD_LINK}'>Ссылка для друзей</a></b>"
        )
    ...
    try:
        ...
        if emoji and winning_values or emoji or winning_values:
            if 'камень' in parsed_data['comment'] or 'ножницы' in parsed_data['comment'] or 'бумага' in parsed_data['comment']:
                ...
                result_message, keyboard = await send_result_message(result, parsed_data, dice_result, COEFFICIENTS[bet_type], us_id, msg_id)
            await asyncio.sleep(1)
            result = 'Победа!' if 'победа' in result_message.lower() or 'выиграли' in result_message.lower() else 'Вы проиграли!'
...
reply_markup=create_keyboard())
                                            ...
                                            if status == 1:
                                                add_text = " ( Сумма умножена на 1.1x )"
                                            else:
                                                add_text = ""
...
<blockquote>Игрок: <code>{parsed_data['name']}</code></blockquote>
<blockquote>Сумма ставки: <code>{parsed_data['usd_amount']:.2f}${add_text}</code></blockquote>
<blockquote>Исход ставки: <code>{parsed_data['comment']}</code></blockquote></b>
...
async def on_startup(_):
    ...
    await bot.send_message(config.LOGS_ID, "⚡ Бот успешно запустился.")

    print("Bot has been started.")



# Регистрация хандлер дп
